#!/usr/bin/env python3
"""Test script to verify the LangGraph agent is working.

Run this script after starting the agent server with:
    cd langgraph_agent && langgraph dev --port 2024
"""
...
async def test_agent(assistant_id, port=2024):
    """Test the LangGraph agent via A2A protocol."""
...
def main():
    ...
    if not assistant_id:
        ...
        print("  export LANGGRAPH_ASSISTANT_ID=<assistant_id>")
        print()
        print("To get the assistant_id:")
        print("  1. Start the agent: cd langgraph_agent && langgraph dev --port 2024")
